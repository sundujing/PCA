...
public class Pca {
	// 将原始数据标准化
	public double[][] Standardlizer(ArrayList<double[]> records) {
		int n = records.size(); // 二维矩阵的行号row
		double[] c=records.get(0);
		int p = c.length; // 二维矩阵的列号col
		double[][] records1=new double[n][p];
		...
		for (int k = 0; k < p; k++) {
			double temp = 0;
			for (int i = 0; i < n; i++) {
				temp += (records1[i][k] - average[k]) * (records1[i][k] - average[k]);
			}
			var[k] = temp / (n - 1);
		}
		// 获得标准化的矩阵
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < p; j++) {
				result[i][j] = (double) ((records1[i][j] - average[j]) / Math
						.sqrt(var[j]));
			}
		}
		return result;
